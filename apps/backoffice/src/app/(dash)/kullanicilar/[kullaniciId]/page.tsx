...
export async function generateMetadata({
  ...
  params: Promise<{ kullaniciId: string }>
}): Promise<Metadata> {
  ...
  return { title: `${userMessages.detail.kicker} ${shortId(kullaniciId)}` }
}
...
export default async function UserDetailPage({
  ...
  if (!isUserId(kullaniciId)) notFound()

  const detail = await loadUserDetail(kullaniciId)
  if (!detail) notFound()
  ...
  const [
    ...
    settle(() => loadUserAccounts(kullaniciId)),
    settle(() => loadUserSyncHealth(kullaniciId)),
    settle(() => loadApprovalBreakdown(kullaniciId)),
    settle(() => loadUserApprovals(kullaniciId)),
    settle(() => loadUserReferral(kullaniciId)),
    settle(() => loadUserPrivacyRequests(kullaniciId)),
    settle(() => loadBriefingHealth(systemClock)),
    settle(() => loadNotificationHealth(systemClock)),
    settle(() => loadRegenerationRequests(kullaniciId)),
    settle(() => loadUserAudit(kullaniciId)),
  ])
