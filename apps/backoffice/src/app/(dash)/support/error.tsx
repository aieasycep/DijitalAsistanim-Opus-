...
/**
 * The support area's error boundary.
 *
 * A failure here is almost always one query being unreachable rather than
 * anything about a ticket, so the operator gets a retry and a way back to the
 * queue — neither of which is a dead end. Nothing from the thrown error is
 * rendered except its digest: a PostgREST message can name the row it failed
 * on, and this tool's promise is that such a string never reaches a screen.
 */
