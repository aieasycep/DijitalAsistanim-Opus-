...
// Imported from the leaf modules rather than the `@/components/audit` barrel:
// this file is a client component, and the barrel also re-exports the server
// components, which reach `@/lib/queries/audit` and through it the service-role
// client. A barrel import here would drag that whole graph across the client
// boundary.
import { AUDIT_PATH } from '@/components/audit/contract'
import { auditMessages } from '@/components/audit/messages'
import { messages } from '@/lib/messages'

/**
 * The audit area's error boundary.
 *
 * It sits at the segment rather than at the root so a thrown query takes down
 * this area and not the whole console, and it offers two ways out: retry the
 * same render, or go back to the unfiltered log, which settles each panel
 * separately and will usually come up with the working ones.
 *
 * It renders nothing from the error but the digest. A PostgREST failure can
 * quote the row it choked on, and on this page of all pages that row is an
 * audit record — the real error is in the server log, keyed by that digest,
