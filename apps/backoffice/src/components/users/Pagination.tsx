...
export function Pagination({
  ...
  return (
    <nav
      aria-label={userMessages.pagination.label}
      className="flex flex-wrap items-center justify-between gap-2 px-1"
    >
      <p className="text-[12px] text-faint">
        {userMessages.pagination.range(formatNumber(from), formatNumber(to), formatNumber(total))}
      </p>

      <div className="flex items-center gap-2">
        <span className="text-[12px] text-faint">
          {userMessages.pagination.page(page, pageCount)}
        </span>
        {page > 1 ? (
          <Link href={href(page - 1)} className={linkClass} rel="prev">
            {userMessages.pagination.previous}
          </Link>
        ) : (
          <span className={edgeClass}>{userMessages.pagination.previous}</span>
        )}
        {page < pageCount ? (
          <Link href={href(page + 1)} className={linkClass} rel="next">
            {userMessages.pagination.next}
          </Link>
        ) : (
          <span className={edgeClass}>{userMessages.pagination.next}</span>
        )}
      </div>
    </nav>
  )
